...
class GatewayPrototypeTest(unittest.TestCase):
    ...
    def test_postman_collection_lists_demo_requests(self):
        ...
        self.assertIn("Migration Plan", admin_names)
        ...
        self.assertIn("Procurement Pack", admin_names)
    ...
    def test_demo_bundle_manifest_lists_handoff_materials(self):
        ...
        self.assertIn(f"{self.base_url}/v1/gateway/migration-plan", entry_urls)
        ...
        self.assertIn(f"{self.base_url}/v1/gateway/procurement-pack", entry_urls)
    ...
    def test_migration_plan_guides_customer_cutover_and_rollback(self):
        status, payload = request_json(self.base_url, path="/v1/gateway/migration-plan", api_key="dev-gateway-key")
        self.assertEqual(status, 401)
        self.assertEqual(payload["error"]["code"], "invalid_admin_key")

        status, plan = request_json(self.base_url, path="/v1/gateway/migration-plan", api_key="dev-admin-key")
        self.assertEqual(status, 200)
        self.assertEqual(plan["object"], "gateway.migration_plan")
        self.assertEqual(plan["migration_strategy"]["recommended_style"], "phased_cutover")
        phases = {phase["phase"] for phase in plan["phases"]}
        self.assertIn("0. Current-state discovery", phases)
        self.assertIn("1. Shadow gateway setup", phases)
        self.assertIn("2. Mock and evaluation test", phases)
        self.assertIn("3. Limited live pilot", phases)
        self.assertIn("4. Gradual cutover", phases)
        self.assertIn("5. Production decision", phases)
        for phase in plan["phases"]:
            self.assertIn("owner", phase)
            self.assertIn("actions", phase)
            self.assertIn("exit_check", phase)
        self.assertTrue(any("Rollback path" in item for item in plan["cutover_checklist"]))
        self.assertTrue(any("old direct provider" in item for item in plan["rollback_plan"]))
        self.assertIn("/v1/gateway/evaluation-plan", plan["evidence_endpoints"])
        self.assertIn("/v1/gateway/change-management", plan["evidence_endpoints"])
        self.assertIn("not need to migrate everything at once", plan["customer_message"])
        self.assertIn("rollback testing", plan["prototype_note"])
        self.assertNotIn("DASHSCOPE_API_KEY", json.dumps(plan))
        self.assertNotIn("sk-", json.dumps(plan))
        self.assertNotIn("provider_api_keys", json.dumps(plan))
    ...
    def test_operations_runbook_lists_slo_checks_and_owner_actions(self):
        status, payload = request_json(self.base_url, path="/v1/gateway/operations-runbook", api_key="dev-gateway-key")
        self.assertEqual(status, 401)
        self.assertEqual(payload["error"]["code"], "invalid_admin_key")

        status, runbook = request_json(self.base_url, path="/v1/gateway/operations-runbook", api_key="dev-admin-key")
        self.assertEqual(status, 200)
        self.assertEqual(runbook["object"], "gateway.operations_runbook")
        target_names = {item["name"] for item in runbook["slo_targets"]}
        self.assertIn("Availability target", target_names)
        self.assertIn("Error rate target", target_names)
        self.assertIn("Latency target", target_names)
        self.assertIn("Budget target", target_names)
        check_names = {item["check"] for item in runbook["daily_checks"]}
        self.assertIn("Gateway health", check_names)
        self.assertIn("Provider health", check_names)
        self.assertIn("Customer errors", check_names)
        owners = {item["role"] for item in runbook["ownership"]}
        self.assertIn("Support owner", owners)
        self.assertIn("Gateway owner", owners)
        self.assertIn("Platform owner", owners)
        self.assertIn("/v1/gateway/incident-playbook", runbook["evidence_endpoints"])
        self.assertIn("/v1/gateway/support-policy", runbook["evidence_endpoints"])
        self.assertIn("not a legal SLA", runbook["prototype_note"])
        self.assertNotIn("DASHSCOPE_API_KEY", json.dumps(runbook))
        self.assertNotIn("sk-", json.dumps(runbook))
        self.assertNotIn("provider_api_keys", json.dumps(runbook))
    ...
    def test_status_exposes_customer_budget_without_provider_secret(self):
        ...
        self.assertIn("procurement_pack", payload)
        self.assertIn("review_tracks", payload["procurement_pack"])
        self.assertIn("business_case", payload)
        self.assertIn("value_hypotheses", payload["business_case"])
        self.assertIn("implementation_plan", payload)
        ...
        self.assertIn("workshop_agenda", payload)
        self.assertIn("agenda", payload["workshop_agenda"])
        self.assertIn("decision_log", payload)
        ...
        self.assertIn("migration_plan", payload)
        self.assertIn("phases", payload["migration_plan"])
        ...
        self.assertIn("operations_runbook", payload)
        self.assertIn("slo_targets", payload["operations_runbook"])
    ...
    def test_admin_summary_endpoints(self):
        ...
        status, procurement = request_json(self.base_url, path="/v1/gateway/procurement-pack")
        self.assertEqual(status, 401)

        status, procurement = request_json(self.base_url, path="/v1/gateway/procurement-pack", api_key="dev-admin-key")
        self.assertEqual(status, 200)
        self.assertEqual(procurement["object"], "gateway.procurement_pack")
        self.assertIn("procurement, legal, IT, security, and finance", procurement["plain_english"])
        tracks = {item["track"] for item in procurement["review_tracks"]}
        self.assertIn("Business sponsor", tracks)
        self.assertIn("Security", tracks)
        self.assertIn("Finance and commercial", tracks)
        documents = {item["document"] for item in procurement["document_checklist"]}
        self.assertIn("OpenAPI contract", documents)
        self.assertIn("Commercial policy", documents)
        self.assertIn("Signed legal terms", documents)
        self.assertIn("Do not expose provider API keys to customers.", procurement["red_lines"])
        self.assertIn("/v1/gateway/security-review", procurement["evidence_endpoints"])
        self.assertNotIn("DASHSCOPE_API_KEY", json.dumps(procurement))
        self.assertNotIn("sk-", json.dumps(procurement))
        self.assertNotIn("provider_api_keys", json.dumps(procurement))

        status, business = request_json(self.base_url, path="/v1/gateway/business-case")
        self.assertEqual(status, 401)

        status, business = request_json(self.base_url, path="/v1/gateway/business-case", api_key="dev-admin-key")
        self.assertEqual(status, 200)
        self.assertEqual(business["object"], "gateway.business_case")
        self.assertIn("worth a pilot", business["plain_english"])
        values = {item["value"] for item in business["value_hypotheses"]}
        self.assertIn("Less integration work", values)
        self.assertIn("Budget visibility", values)
        self.assertIn("Clearer internal approval", values)
        self.assertIn("requests_recorded", business["pilot_metrics"])
        self.assertIn("engineering days", " ".join(business["roi_inputs_to_collect"]))
        options = {item["option"] for item in business["decision_options"]}
        self.assertIn("Run a small pilot", options)
        self.assertIn("Guaranteed cost savings", business["not_claimed"])
        self.assertIn("/v1/gateway/procurement-pack", business["evidence_endpoints"])
        self.assertNotIn("DASHSCOPE_API_KEY", json.dumps(business))
        self.assertNotIn("sk-", json.dumps(business))
        self.assertNotIn("provider_api_keys", json.dumps(business))

        status, implementation = request_json(self.base_url, path="/v1/gateway/implementation-plan")
        self.assertEqual(status, 401)

        status, implementation = request_json(self.base_url, path="/v1/gateway/implementation-plan", api_key="dev-admin-key")
        self.assertEqual(status, 200)
        self.assertEqual(implementation["object"], "gateway.implementation_plan")
        self.assertIn("delivery steps", implementation["plain_english"])
        phases = {item["phase"] for item in implementation["delivery_phases"]}
        self.assertIn("Phase 0: Discovery and scope", phases)
        self.assertIn("Phase 3: Production hardening", phases)
        roles = {item["role"] for item in implementation["role_plan"]}
        self.assertIn("Business sponsor", roles)
        self.assertIn("Security owner", roles)
        self.assertIn("Finance/legal owner", roles)
        self.assertIn("Fixed implementation price", implementation["not_claimed"])
        self.assertIn("/v1/gateway/production-backlog", implementation["evidence_endpoints"])
        self.assertIn("production_readiness_status", implementation["current_context"])
        self.assertNotIn("DASHSCOPE_API_KEY", json.dumps(implementation))
        self.assertNotIn("sk-", json.dumps(implementation))
        self.assertNotIn("provider_api_keys", json.dumps(implementation))

        status, alternatives = request_json(self.base_url, path="/v1/gateway/alternatives-pack")
        self.assertEqual(status, 401)

        status, alternatives = request_json(self.base_url, path="/v1/gateway/alternatives-pack", api_key="dev-admin-key")
        self.assertEqual(status, 200)
        self.assertEqual(alternatives["object"], "gateway.alternatives_pack")
        self.assertIn("OpenRouter-like", alternatives["plain_english"])
        options = {item["option"] for item in alternatives["alternatives"]}
        self.assertIn("Normal API Gateway", options)
        self.assertIn("Vercel AI Gateway", options)
        self.assertIn("OpenRouter-like platform", options)
        self.assertIn("Custom AISmallRouter-style Model Gateway", options)
        self.assertIn("Complete OpenRouter clone", alternatives["not_claimed"])
        self.assertIn("/v1/gateway/decision-guide", alternatives["evidence_endpoints"])
        sources = {item["name"] for item in alternatives["reference_sources"]}
        self.assertIn("OpenRouter documentation", sources)
        self.assertIn("Vercel AI Gateway documentation", sources)
        self.assertIn("Alibaba Cloud AI Gateway documentation", sources)
        self.assertNotIn("DASHSCOPE_API_KEY", json.dumps(alternatives))
        ...
        status, workshop = request_json(self.base_url, path="/v1/gateway/workshop-agenda")
        self.assertEqual(status, 401)

        status, workshop = request_json(self.base_url, path="/v1/gateway/workshop-agenda", api_key="dev-admin-key")
        self.assertEqual(status, 200)
        self.assertEqual(workshop["object"], "gateway.workshop_agenda")
        self.assertIn("first customer workshop", workshop["plain_english"])
        attendees = {item["role"] for item in workshop["attendees"]}
        self.assertIn("Customer sponsor", attendees)
        self.assertIn("Security or data owner", attendees)
        topics = {item["topic"] for item in workshop["agenda"]}
        self.assertIn("Gateway concept demo", topics)
        self.assertIn("Pilot shape", topics)
        self.assertIn("Confirmed first use case.", workshop["expected_outputs"])
        self.assertIn("No named customer sponsor.", workshop["red_flags"])
        self.assertIn("/v1/gateway/sow-draft", workshop["evidence_endpoints"])
        self.assertNotIn("DASHSCOPE_API_KEY", json.dumps(workshop))
        self.assertNotIn("sk-", json.dumps(workshop))
        self.assertNotIn("provider_api_keys", json.dumps(workshop))

        status, decisions = request_json(self.base_url, path="/v1/gateway/decision-log")
        self.assertEqual(status, 401)

        status, decisions = request_json(self.base_url, path="/v1/gateway/decision-log", api_key="dev-admin-key")
        self.assertEqual(status, 200)
        self.assertEqual(decisions["object"], "gateway.decision_log")
        self.assertIn("customer workshop", decisions["plain_english"])
        decision_names = {item["decision"] for item in decisions["decision_records"]}
        self.assertIn("First use case", decision_names)
        self.assertIn("Mock or live pilot", decision_names)
        self.assertIn("Build, buy, or managed gateway option", decision_names)
        self.assertIn("Who is the named customer sponsor?", decisions["open_questions"])
        owners = {item["owner"] for item in decisions["next_actions"]}
        self.assertIn("Gateway owner", owners)
        self.assertIn("Customer sponsor", owners)
        self.assertIn("Date:", decisions["meeting_note_template"])
        self.assertIn("/v1/gateway/workshop-agenda", decisions["evidence_endpoints"])
        self.assertNotIn("DASHSCOPE_API_KEY", json.dumps(decisions))
        self.assertNotIn("sk-", json.dumps(decisions))
        self.assertNotIn("provider_api_keys", json.dumps(decisions))

        status, follow_up = request_json(self.base_url, path="/v1/gateway/follow-up-email")
        self.assertEqual(status, 401)

        status, follow_up = request_json(self.base_url, path="/v1/gateway/follow-up-email", api_key="dev-admin-key")
        self.assertEqual(status, 200)
        self.assertEqual(follow_up["object"], "gateway.follow_up_email")
        self.assertIn("customer follow-up email", follow_up["title"].lower())
        self.assertIn("Thank you", " ".join(follow_up["customer_email"]["body"]))
        self.assertIn("AISmallRouter workshop recap and next steps", follow_up["subject_options"])
        link_names = {item["name"] for item in follow_up["links_to_include"]}
        self.assertIn("Visual dashboard", link_names)
        self.assertIn("Decision log", link_names)
        action_owners = {item["owner"] for item in follow_up["next_actions"]}
        self.assertIn("Gateway owner", action_owners)
        self.assertIn("Signed contract", follow_up["not_claimed"])
        self.assertIn("/v1/gateway/decision-log", follow_up["evidence_endpoints"])
        self.assertNotIn("DASHSCOPE_API_KEY", json.dumps(follow_up))
        ...
        status, review = request_json(self.base_url, path="/v1/gateway/pilot-review")
        self.assertEqual(status, 401)

        status, review = request_json(self.base_url, path="/v1/gateway/pilot-review", api_key="dev-admin-key")
        self.assertEqual(status, 200)
        self.assertEqual(review["object"], "gateway.pilot_review")
        self.assertIn("stop, extend the pilot, or move toward production hardening", review["plain_english"])
        input_names = {item["input"] for item in review["review_inputs"]}
        self.assertIn("Pilot kickoff", input_names)
        self.assertIn("Production readiness", input_names)
        options = {item["option"] for item in review["decision_options"]}
        self.assertIn("Stop", options)
        self.assertIn("Extend pilot", options)
        self.assertIn("Move to production hardening", options)
        checks = {item["check"] for item in review["go_no_go_checks"]}
        self.assertIn("Prompt logging and retention answer approved", checks)
        self.assertIn("Production approval", review["not_claimed"])
        self.assertIn("/v1/gateway/production-backlog", review["evidence_endpoints"])
        self.assertNotIn("DASHSCOPE_API_KEY", json.dumps(review))
        self.assertNotIn("sk-", json.dumps(review))
        self.assertNotIn("provider_api_keys", json.dumps(review))
        ...
        status, expansion = request_json(self.base_url, path="/v1/gateway/provider-expansion")
        self.assertEqual(status, 401)

        status, expansion = request_json(self.base_url, path="/v1/gateway/provider-expansion", api_key="dev-admin-key")
        self.assertEqual(status, 200)
        self.assertEqual(expansion["object"], "gateway.provider_expansion")
        self.assertIn("add more model providers", expansion["plain_english"])
        candidates = {item["provider"] for item in expansion["provider_candidates"]}
        self.assertIn("Alibaba Cloud Model Studio / Qwen", candidates)
        self.assertIn("Anthropic / Claude", candidates)
        gates = {item["gate"] for item in expansion["approval_gates"]}
        self.assertIn("Technical contract", gates)
        self.assertIn("Security and data", gates)
        tests = {item["test"] for item in expansion["test_matrix"]}
        self.assertIn("Route preview", tests)
        self.assertIn("Fallback and rollback", tests)
        self.assertIn("Full OpenRouter clone", expansion["not_claimed"])
        self.assertIn("/v1/gateway/provider-contracts", expansion["evidence_endpoints"])
        self.assertNotIn("DASHSCOPE_API_KEY", json.dumps(expansion))
    ...
    def test_dashboard_exposes_presenter_mode(self):
        ...
        self.assertIn("Operations runbook", html)
        self.assertIn("operationsRunbookList", html)
        self.assertIn("/v1/gateway/operations-runbook?admin_key=", html)
        ...
        self.assertIn("Migration plan", html)
        self.assertIn("migrationPlanList", html)
        self.assertIn("/v1/gateway/migration-plan?admin_key=", html)
        ...
        self.assertIn("Procurement pack", html)
        self.assertIn("procurementPackList", html)
        self.assertIn("/v1/gateway/procurement-pack?admin_key=", html)
        self.assertIn("Business case", html)
        self.assertIn("businessCaseList", html)
        self.assertIn("/v1/gateway/business-case?admin_key=", html)
        self.assertIn("Implementation plan", html)
        self.assertIn("implementationPlanList", html)
        self.assertIn("/v1/gateway/implementation-plan?admin_key=", html)
        ...
        self.assertIn("Workshop agenda", html)
        self.assertIn("workshopAgendaList", html)
        self.assertIn("/v1/gateway/workshop-agenda?admin_key=", html)
        self.assertIn("Decision log", html)
        self.assertIn("decisionLogList", html)
        self.assertIn("/v1/gateway/decision-log?admin_key=", html)
